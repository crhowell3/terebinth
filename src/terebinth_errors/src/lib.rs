//! This is the errors and diagnostics creator and emitter for the Terebinth
//! compiler binary.

extern crate self as terebinth_errors;
...
use terebinth_data_structures::AtomicRef;
use terebinth_data_structures::fx::{FxHashSet, FxIndexMap, FxIndexSet};
use terebinth_data_structures::stable_hasher::StableHasher;
use terebinth_data_structures::sync::{DynSend, Lock};
pub use terebinth_error_messages::{
    DiagMessage, FluentBundle, LanguageIdentifier, LazyFallbackBundle, MultiSpan, SpanLabel,
    SubdiagMessage, fallback_fluent_bundle, fluent_bundle,
};
use terebinth_hashes::Hash128;
...
terebinth_data_structures::static_assert_size!(PResult<'_, ()>, 24);
#[cfg(target_pointer_width = "64")]
terebinth_data_structures::static_assert_size!(PResult<'_, bool>, 24);
...
impl CodeSuggestion {
    pub(crate) fn splice_lines(
        ...
        String,
        Vec<SubstitutionPart>,
        Vec<Vec<SubstitutionHighlight>>,
        bool,
    )> {
        use terebinth_span::{CharPos, Pos};
...
/// A `DiagCtx` deals with errors and other compiler output.
/// Certain errors (fatal, bug, unimpl) may cause immediate exit,
/// others log errors for later reporting.
pub struct DiagCtx {
    inner: Lock<DiagCtxInner>,
}

#[derive(Copy, Clone)]
pub struct DiagCtxHandle<'a> {
    dcx: &'a DiagCtx,
    /// Some contexts create `DiagCtxHandle` with this field set, and thus all
...
impl<'a> std::ops::Deref for DiagCtxHandle<'a> {
    type Target = &'a DiagCtx;
...
struct DiagCtxInner {
    ...
    /// this `DiagCtx`. These hashes is used to avoid emitting the same error
    /// twice.
    ...
    /// `emit_stashed_diagnostics` by the time the `DiagCtxInner` is dropped,
...
impl DiagCtx {
    ...
    pub fn with_flags(mut self, flags: DiagCtxFlags) -> Self {
    ...
    pub fn new(emitter: Box<DynEmitter>) -> Self {
        Self {
            inner: Lock::new(DiagCtxInner::new(emitter)),
        }
    }
    ...
    pub fn handle<'a>(&'a self) -> DiagCtxHandle<'a> {
        DiagCtxHandle {
            dcx: self,
...
impl<'a> DiagCtxHandle<'a> {
        ...
        let guar = match diag.level {
            ...
            DelayedBug => {
                return self
                    .inner
                    .borrow_mut()
                    .emit_diagnostic(diag, self.tainted_with_errors);
            }
    ...
    pub fn err_count(&self) -> usize {
        ...
        inner.err_guars.len()
            ...
            + inner
                .stashed_diagnostics
                .values()
                .filter(|(_diag, guar)| guar.is_some())
                .count()
    }
    ...
    pub fn emit_diagnostic(&self, diagnostic: DiagInner) -> Option<ErrorGuaranteed> {
        self.inner
            .borrow_mut()
            .emit_diagnostic(diagnostic, self.tainted_with_errors)
    }
    ...
    pub fn emit_artifact_notification(&self, path: &Path, artifact_type: &str) {
        self.inner
            .borrow_mut()
            .emitter
            .emit_artifact_notification(path, artifact_type);
    }
    ...
    pub fn emit_future_breakage_report(&self) {
        ...
        if !diags.is_empty() {
            inner
                .emitter
                .emit_future_breakage_report(diags, &inner.registry);
        }
    }
    ...
    pub fn emit_unused_externs(
        ...
        if loud && lint_level.is_error() {
            ...
            inner
                .lint_err_guars
                .push(ErrorGuaranteed::unchecked_error_guaranteed());
    ...
    /// [`DiagCtxInner`] and indicate that the linked expectation has been fulfilled.
    #[must_use]
    ...
    fn flush_delayed(&mut self) {
        ...
        for bug in bugs {
            ...
            if bug.level != DelayedBug {
                ...
                //
                // We are at the `DiagInner`/`DiagCtxInner` level rather than
                // the usual `Diag`/`DiagCtx` level, so we must augment `bug`
...
impl DelayedDiagInner {
    ...
    fn decorate(self, dcx: &DiagCtxInner) -> DiagInner {
        // We are at the `DiagInner`/`DiagCtxInner` level rather than the
        // usual `Diag`/`DiagCtx` level, so we must construct `diag` in a
        // lower-level fashion.
