...
use terebinth_hashes::Hash64;
use terebinth_index::IndexVec;
...
impl fmt::Debug for ExpnId {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{{{{expn{}}}}}", self.local_id.as_u32())
    }
}
...
impl HygieneData {
    ...
    fn alloc_ctxt(
        ...
        self.syntax_context_data
            .push(SyntaxContextData::decode_placeholder());
...
pub fn decode_expn_id(
    ...
    let expn_id = ExpnId { local_id: index };
...
>(
    ...
    let ctxt = HygieneData::with(|hygiene_data| {
        let ctxt_key = (
            ctxt_data.parent,
            ctxt_data.outer_expn,
            ctxt_data.outer_transparency,
        );
        *hygiene_data
            .syntax_context_map
            .entry(ctxt_key)
...
fn for_all_ctxts_in<F: FnMut(u32, SyntaxContext, &SyntaxContextDataNonRecursive)>(
    ...
    let all_data: Vec<_> = HygieneData::with(|data| {
        ctxts
            .map(|ctxt| (ctxt, data.non_recursive_ctxt(ctxt)))
            .collect()
    });
