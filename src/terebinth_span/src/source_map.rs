...
pub fn original_sp(sp: Span, enclosing_sp: Span) -> Span {
    ...
    let enclosing_ctx = enclosing_sp.ctx();
    let expn_data1 = ctx.outer_expn_data();
    if !enclosing_ctx.is_root() && expn_data1.call_site == enclosing_ctx.outer_expn_data().call_site
    {
        sp
    } else {
