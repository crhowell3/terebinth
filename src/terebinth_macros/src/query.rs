...
fn add_query_desc_cached_impl(
    ...
    let cache = if let Some((args, expr)) = modifiers.cache.as_ref() {
        ...
        // we're taking `key` by reference, but some terebinth types usually prefer being passed by value
        quote! {
            #[allow(unused_variables, unused_braces, terebinth::pass_by_value)]
            #[inline]
    ...
    let desc = quote! {
        ...
        pub fn #name<'tcx>(tcx: TyCtxt<'tcx>, key: crate::query::queries::#name::Key<'tcx>) -> String {
            let (#tcx, #key) = (tcx, key);
            ::terebinth_middle::ty::print::with_no_trimmed_paths!(
                format!(#desc)
            )
        }
    };
...
pub(super) fn terebinth_queries(input: TokenStream) -> TokenStream {
    ...
    for query in queries.0 {
        ...
        // at the entire `terebinth_queries!` invocation, which wouldn't
        // be very useful.
        let span = name.span();
    ...
    TokenStream::from(quote! {
        #[macro_export]
        macro_rules! terebinth_query_append {
        ...
        macro_rules! terebinth_feedable_queries {
            ( $macro:ident! ) => {
