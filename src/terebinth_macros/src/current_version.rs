...
pub(crate) fn current_version(_input: TokenStream) -> TokenStream {
    ...
    TokenStream::from(match TerebinthVersion::parse_cfg_release(env_var) {
        Ok(TerebinthVersion {
            major,
            minor,
            patch,
        }) => quote!(
            // The produced literal has type `session::TerebinthVersion`.
